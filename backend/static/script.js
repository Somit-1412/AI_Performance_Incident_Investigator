async function startTest() {
    ...
    resultsDiv.innerHTML = "Running load test...";

    try {

        const response = await fetch(
            `http://localhost:5000/start-test?baseUrl=${encodeURIComponent(baseUrl)}&path=${encodeURIComponent(path)}&users=${users}&rampup=${rampup}&loops=${loops}`
        );
